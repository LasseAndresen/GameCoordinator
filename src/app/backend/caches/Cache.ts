...
export class Cache implements ICache {
  ...
  public collectionObservable: BehaviorSubject<IDataBaseEntity[]> = new BehaviorSubject<IDataBaseEntity[]>([]);
  ...
  public clearCache(): void {
    ...
    Object.values(this.cachedCallbackFunctions).forEach(f => f.forEach(ff => ff()));
  ...
  protected _addObject(object: IDataBaseEntity, cache: {[guid: string]: any}, observable: BehaviorSubject<any[]>) {
  ...
  protected _removeObject(guid: string, cache: {[guid: string]: any}, observable: BehaviorSubject<any[]>) {
    delete cache[guid];
    const collectionCopy = [...observable.value];

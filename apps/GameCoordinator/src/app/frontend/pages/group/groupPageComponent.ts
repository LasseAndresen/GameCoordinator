...
export class GroupPageComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    this._uiSubscriptions.push(
      this._router.events
        ...
        .subscribe((event) => this.onNavigationChange(event))
    );
    this.reload();
    this._uiSubscriptions.push(
      this._applicationContext.requestAppReload.subscribe(() => this.reload())
    );
  }
  ...
  public onFavoriteGameClicked(game: BoardGame) {
    ...
    this._firestoreService.toggleFavoriteGroupBoardGame(
      game.guid,
      this.view.group.guid,
      game.isFavorite
    );
  }
  ...
  public onNewPostClicked() {
    this._addGroupPostDialog.show(
      this.view.group.guid,
      this.view.members.map((m) => m.name),
      null
    );
  }
}

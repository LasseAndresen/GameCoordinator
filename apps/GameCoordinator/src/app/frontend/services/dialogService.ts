...
export class DialogService {
  ...
  public showDialog<T>(component: Type<T>, providers: ValueProvider[]) {
    const injector: Injector = Injector.create({ providers: [...providers] });
    const factory = this._cfr.resolveComponentFactory(component);
    const cr: ComponentRef<T> =
      this._applicationContext.appMainContainer.createComponent(
        factory,
        0,
        injector
      );
    const renderer = this._rendererFactory.createRenderer(
      cr.location.nativeElement,
      null
    );
    ...
    renderer.setStyle(
      cr.location.nativeElement,
      'z-index',
      this._dialogComponentRefs.length + 1
    );

...
export class GroupFactory implements IDataBaseEntityFactory {
  public fromDbObject(
    groupData: QueryDocumentSnapshot<any>,
    memberData: QuerySnapshot<any>,
    boardGameData: QuerySnapshot<any>
  ): Group {
    ...
    if (memberData != null) {
      ...
      memberData.forEach((doc) => {
        if (doc.id !== null) {
          groupMembers.push(doc.data().name);
        }
      });
    ...
    if (boardGameData != null) {
      const boardGames = [];
      boardGameData.forEach((doc) => {
        if (doc.id !== null) {
          boardGames.push(doc.data().name);
        }
      });
...
export class Group implements IDataBaseEntity<Group> {
  ...
  public updateMembers(memberData: QuerySnapshot<any>) {
    ...
    memberData.forEach((doc) => {
      if (doc.id !== null) {
        groupMembers.push(factory.fromDbObject(doc));
      }
    });
  ...
  public updateBoardGames(boardGameData: QuerySnapshot<any>) {
    ...
    boardGameData.forEach((doc) => {
      if (doc.id !== null) {
        boardGames.push(factory.fromDbObject(doc));
      }
    });
  ...
  public updatePosts(postData: QuerySnapshot<any>) {
    ...
    postData.forEach((doc) => {
      if (doc.id !== null) {
        posts.push(factory.fromDbObject(doc));
      }
    });
